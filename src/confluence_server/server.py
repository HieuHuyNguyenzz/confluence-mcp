...
from confluence_server.client import ConfluenceClient
from confluence_server.converter import (
    convert_page_to_markdown,
    generate_page_path,
)
from confluence_server.extractor import FileExtractor
from confluence_server.processor import process_page, process_attachment, is_binary_type
...
class ClientManager:
    """Manage lifecycle of API clients."""
    def __init__(self):
...
async def convert_file_to_markdown(file_path: str) -> str:
    ...
    try:
        ...
        return {"error": f"Error converting file: {type(e).__name__}: {e}"}
...
async def list_spaces(limit: int = 25, start: int = 0) -> list[dict[str, Any]]:
    ...
    try:
        ...
        return [{"error": f"Error listing spaces: {e}"}]
